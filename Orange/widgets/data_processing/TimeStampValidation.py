...
from tods.data_processing.TimeStampValidation import *

class OWTimeStampValidation(SingleInputWidget):
    name = "TimeStampValidation"
    description = (
        "    Checks if time series is sorted by time stamp , if not then return sorted time series")
    icon = "icons/TimeStampValidation.svg"
    ...
    primitive = TimeStampValidationPrimitive
    ...
    def _init_ui(self):
        ...
        # use_columns = Setting(())
        gui.lineEdit(box, self, "use_columns_buf",
                     label='Column index to use when use_semantic_types is activated. Tuple, e.g. (0,1,2)',
                     validator=None, callback=self._use_columns_callback)

        # exclude_columns = Setting(())
        gui.lineEdit(box, self, "exclude_columns_buf",
                     label='Column index to exclude when use_semantic_types is activated. Tuple, e.g. (0,1,2)',
...
if __name__ == "__main__":
    WidgetPreview(OWTimeStampValidation).run(Orange.data.Table("iris"))
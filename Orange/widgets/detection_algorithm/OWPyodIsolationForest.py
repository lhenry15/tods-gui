...
class OWPyodIsolationForest(SingleInputWidget):
    name = "PyodIsolationForest"
    description = ("IsolationForest class for outlier detection.")
    icon = "icons/isolation_forest.svg"
    # icon = "icons/File.svg"
    ...
    max_samples = Setting('auto')
    max_features = Setting(1.)
    bootstrap = Setting(False)
    ...
    primitive = IsolationForestPrimitive
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(box, self, 'n_estimators', label='Number of base estimators in the ensemble. (IntVariable)', callback=None)

        gui.comboBox(box, self, 'max_samples', label='Number of samples to draw from training data for each base estimator.',
                     items=['auto', 'int', 'float'], callback=None)

        gui.lineEdit(box, self, 'max_features', label='Number of features to draw from training data for each base estimator. (Float)', callback=None)

        gui.checkBox(box, self, "bootstrap", label='Fit on random subsets of training data.',  callback=None)

        gui.lineEdit(box, self, 'random_state', label='Seed used by the random number generator. (Int or None)', callback=None)

        gui.lineEdit(box, self, 'verbose', label='Controls the verbosity of the tree building process. (IntVariable)', callback=None)

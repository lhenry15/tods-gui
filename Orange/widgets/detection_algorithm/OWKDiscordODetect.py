...
from tods.detection_algorithm.KDiscordODetect import *

class OWKDiscordODetect(SingleInputWidget):
    name = "KDiscordODetect"
    description = ("KDiscord models for outlier detection.")
    icon = "icons/KDiscord.svg"
    ...
    primitive = KDiscordODetectorPrimitive
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(box, self, 'n_neighbors',
                     label='Number of neighbors to use by default for k neighbors queries. (IntVariable)',
                     callback=None)

        gui.comboBox(box, self, 'method', label='Method to calculate outlier score.',
                     items=['largest', 'mean', 'median'], callback=None)

        gui.lineEdit(box, self, 'radius',
                     label='Range of parameter space to use by default for radius_neighbors queries. (Float)',
                     callback=None)
        ...
        gui.lineEdit(box, self, 'leaf_size', label='Leaf size passed to BallTree or KDTree. (IntVariable)',
                     callback=None)

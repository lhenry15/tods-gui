...
from tods.detection_algorithm.PyodLODA import *

class OWPyodLODA(SingleInputWidget):
    name = "PyodLODA"
    description = ("Lightweight on-line detector of anomalies (LODA) for outlier detection.")
    icon = "icons/LODA.svg"
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(box, self, 'n_bins', label='The number of bins for the histogram. (IntVariable)', callback=None)

        gui.lineEdit(box, self, 'n_random_cuts', label='The number of random cuts. (IntVariable)', callback=None)

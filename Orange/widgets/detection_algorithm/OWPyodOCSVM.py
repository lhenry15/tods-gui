...
from tods.detection_algorithm.PyodOCSVM import *

class OWPyodOCSVM(SingleInputWidget):
    name = "PyodOCSVM"
    description = ("One-class SVM for outlier detection.")
    icon = "icons/ocsvm.svg"
    ...
    coef0 = Setting(0.)
    tol = Setting(0.001)
    shrinking = Setting(True)
    cache_size = Setting(200)
    verbose = Setting(False)
    max_iter = Setting(-1)
    ...
    algorithm_metric = Setting('minkowski')
    p = Setting(2)

...
from tods.detection_algorithm.PyodCBLOF import *

class OWPyodCBLOF(SingleInputWidget):
    name = "PyodCBLOF"
    description = ("Cluster-based local outlier factor (CBLOF).")
    icon = "icons/CBLOF.svg"
    ...
    n_clusters =  Setting(8)
    alpha = Setting(0.9)
    beta = Setting(5)
    use_weights = Setting(False)
    check_estimator = Setting(False)
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(box, self, 'n_clusters',
                     label='Number of clusters to form as well as the number of centroids to generate. (IntVariable)',
                     callback=None)
        ...
        gui.doubleSpin(
            ...
            maxv=1.,
            step=0.01,
            label="Coefficient for deciding small and large clusters. [0.5,1.].",
            # callbackOnReturn = True,
            # checked = "BoundedFloat"
        )

        gui.lineEdit(box, self, 'beta',
                     label='Coefficient for deciding small and large clusters. (IntVariable)',
                     callback=None)

        gui.checkBox(box, self, "use_weights", label='Size of clusters used as weights in outlier score calculation.',  callback=None)

        gui.checkBox(box, self, "check_estimator", label='Check whether the base estimator is consistent with sklearn standard.',  callback=None)

        gui.lineEdit(box, self, 'random_state',
                     label='The seed used by the random number generator. (IntVariable or None)',
                     callback=None)

...
class OWAutoRegODetect(SingleInputWidget):
    name = "AutoRegODetect"
    description = ("Autoregressive models for outlier detection.")
    ...
    method = Setting('average')
    weights = Setting(None)
    ...
    primitive = AutoRegODetectorPrimitive
    ...
    def _init_ui(self):
        ...
        gui.comboBox(
            box, self, 'method', label='Combination method.',
            items=['linear', 'poly', 'rbf', 'sigmoid', 'precomputed'], callback=None)

        gui.lineEdit(box, self, 'weights', label='Score weight by dimensions. (List or None)', callback=None)

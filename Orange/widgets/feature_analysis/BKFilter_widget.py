...
from tods.feature_analysis.BKFilter import *

class OWBKFilter(SingleInputWidget):
    name = "BKFilter"
    ...
    def _init_ui(self):
        ...
        gui.lineEdit(
            box, self, "low", label='low', valueType=int,
            callback=None)
        gui.lineEdit(
            box, self, "high", label='high', valueType=int,
            callback=None)
        gui.lineEdit(
            box, self, "key", label='key', valueType=int,
            callback=None)
...
if __name__ == "__main__":
    WidgetPreview(OWBKFilter).run(Orange.data.Table("iris"))